...
from models import SextoSurvey
from database import db                                          # importa la db desde database.py
from utils.sexto_survey_utils import obtener_y_guardar_survey
...
sexto_survey_bp = Blueprint('sexto_survey_bp', __name__)     # instanciar admin_bp desde clase Blueprint para crear las rutas.
bcrypt = Bcrypt()
jwt = JWTManager()
...
@sexto_survey_bp.before_request
def authorize():
    if request.method == 'OPTIONS':
        return
    if request.path in ['/descargar_sexto_survey','/recuperar_sexto_survey','/test_encuestas_cursos_bp','/','/correccion_campos_vacios','/descargar_positividad_corregida','/download_comments_evaluation','/all_comments_evaluation','/download_resume_csv','/create_resumes_of_all','/descargar_excel','/create_resumes', '/reportes_disponibles', '/create_user', '/login', '/users','/update_profile','/update_profile_image','/update_admin']:
        return
...
@sexto_survey_bp.route('/test_sexto_survey_bp', methods=['GET'])
def test():
    return jsonify({'message': 'test bien sucedido','status':"Si lees esto, las rutas de sexto_survey funciona okkk..."}),200


# RUTAS SURVEY NUEVAS ( PEDIDO Y RECUPERACION )-------------------------------------------------------------//////////////////////////
@sexto_survey_bp.route('/recuperar_sexto_survey', methods=['GET'])
def obtener_y_guardar_survey_ruta():
    from extensions import executor
    logger.info("0 - GET > /recuperar_sexto_survey a comenzando...")
...
@sexto_survey_bp.route('/descargar_sexto_survey', methods=['GET'])
def descargar_sexto_survey():
    try:
        # Obtener el registro más reciente de la base de datos
        survey_record = SextoSurvey.query.order_by(SextoSurvey.id.desc()).first()
        ...
        return send_file(output, download_name='sexto_survey_respuestas.xlsx', as_attachment=True, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
